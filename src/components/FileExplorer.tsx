...
export default function FileExplorer({ onInsert }: FileExplorerProps) {
  ...
  const handleView = useCallback((filePath: string) => {
    // Delegate to the shared FileViewer (App listens for 'view-file'), which
    // handles images, markdown preview, text, and open-in-new-tab types.
    window.dispatchEvent(new CustomEvent('view-file', { detail: { path: filePath } }));
  }, []);
